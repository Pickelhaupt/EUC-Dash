...
#include "config/dashboardconfig.h"
...
dashboardconfig_t dashboardconfig;
...
void set_arc_properties() {
    if (dashboardconfig.simple) {
        batt_arc_start = 65;
        batt_arc_end = 295;
        rev_batt_arc = false;
        arclinew = 25; // line width of arc gauges
    }
    else if (dashboardconfig.medium) {
        batt_arc_start = 110;
        batt_arc_end = 250;
        rev_batt_arc = false;
...
void lv_create_temp_arc(void)
{
    ...
    //gui_add_slide_element(temp_arc);
    ...
    //gui_add_slide_element(temp_max_bar);
}
...
void lv_create_overlay(void)
{
    ...
    gui_add_slide_element(overlay_bar);
    ...
    gui_add_slide_element(overlay_label);
...
void dashboard_speed_update(float current_speed, float warn_speed, float tiltback_speed, float top_speed, float avg_speed)
{
    ...
    if (dashboardconfig.full && speed_arc != NULL) {
        lv_obj_refresh_style(speed_arc, LV_ARC_PART_INDIC, LV_STYLE_LINE_COLOR);
        lv_arc_set_range(speed_arc, 0, (tiltback_speed + 5));
        lv_arc_set_value(speed_arc, current_speed);
        if (dashboardconfig.bars && speed_max_bar != NULL && speed_avg_bar != NULL) {
    ...
    if (dashboardconfig.impdist) current_speed = current_speed / 1.6;
...
void dashboard_batt_update(float current_battpct, float min_battpct, float max_battpct)
{
    ...
    if (dashboardconfig.bars && batt_max_bar != NULL && batt_min_bar != NULL ) {
    ...
    if (dashboardconfig.full && batt_label != NULL ) {
...
{
    if (current_arc == NULL || dashboardconfig.simple) return;
    ...
    if (dashboardconfig.bars && current_max_bar != NULL && current_regen_bar != NULL) {
    ...
    if (dashboardconfig.full && current_label != NULL) {
...
{
    if (temp_arc == NULL || !dashboardconfig.full) return;
    ...
    if (dashboardconfig.bars && temp_max_bar != NULL) {
...
{
   if (trip != NULL && dashboardconfig.full)
    {
        char tripstring[6];
        if (dashboardconfig.impdist)
        {
            current_trip = current_trip / 1.6;
        }
...
static void lv_time_task(lv_task_t *time_task)
{
    ...
    if (dashboardconfig.timedisplay) updateTime();
}
...
void dashboard_setup() {
    ...
    if (!dashboardconfig.simple) lv_create_current_arc();
    lv_create_batt_arc();
    if (dashboardconfig.timedisplay) lv_create_dashtime();
    lv_create_overlay();
}

void dashboard_reload(void)
{
    ...
    else if (dashboardconfig.simple) {
...
void dashboard_cont_cb(lv_obj_t * obj, lv_event_t event) {
    Serial.printf("dashboard_cont_cb event: %d \n", event);
    /*
...
{
    dashboard_tile_num = gui_add_tile(1, 0, "fd tile");
    dashboard_cont = gui_get_tile_obj(dashboard_tile_num);
    style = gui_get_style();
    dashboard_read_config();
    log_i("setting up dashboard");
    dashboard_reload();
    //lv_group_add_obj(button_group, dashboard_cont);
    //lv_obj_set_event_cb( dashboard_cont, dashboard_cont_cb);

    gui_add_tile_activate_cb(dashboard_tile_num, dashboard_activate_cb);
    gui_add_tile_hibernate_cb(dashboard_tile_num, dashboard_hibernate_cb);
}